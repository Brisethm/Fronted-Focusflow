      ...
      <div class="bg-card-light dark:bg-card-dark w-full max-w-2xl max-h-[90vh] rounded-2xl shadow-xl flex flex-col overflow-hidden">
        ...
        <div ref="chatContainer" class="flex-1 overflow-y-auto p-5 space-y-6 chat-scroll bg-white">
          ...
          <div class="space-y-4">
            ...
            <div v-else-if="responses.length === 0" class="text-center text-sm text-gray-500 py-4">
              No hay respuestas aún. Escribe el primer mensaje.
            </div>
            ...
            <div v-else class="space-y-4">
              ...
              >
                <span class="text-[10px] font-semibold text-gray-500 mb-1 px-1">
                  {{ resp.esSoporte ? 'Equipo de Soporte' : 'Usuario' }} • {{ formatTime(resp.fecha) }}
                </span>
                <div
        ...
        <div class="p-4 border-t border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark shrink-0">
          <div
            ...
            class="text-center py-2 text-sm font-semibold text-text-light-secondary dark:text-text-dark-secondary flex items-center justify-center gap-2"
          >
            <span class="material-symbols-outlined text-lg">lock</span>
            Este ticket se encuentra cerrado. No se admiten más respuestas.
          </div>
  ...
  methods: {
    ...
    async loadResponses(silent = false) {
      ...
      try {
        ...
        if (!silent) this.toast.error("No se pudieron cargar las respuestas.");
      } finally {
    ...
    async handleSendResponse() {
      ...
      try {
        ...
        if (this.localTicket.estado === "open") {
        ...
        this.toast.error("Error al enviar la respuesta. Inténtalo de nuevo.");
      } finally {
        this.sending = false;
      }
    },
...
.chat-scroll::-webkit-scrollbar-thumb  { background-color: rgba(156,163,175,0.4); border-radius: 10px; }
.chat-scroll::-webkit-scrollbar-thumb:hover { background-color: rgba(156,163,175,0.7); }
</style>
