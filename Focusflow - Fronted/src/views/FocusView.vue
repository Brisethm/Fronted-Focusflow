    ...
    <div class="timer-section">
        ...
        <div class="timer-inner">
          <template v-if="completed">
            <div class="completed-badge fade-in">
              ...
              <p class="done-label" :style="{ color: accent }">
                {{ t("focus.completed") }}
              </p>
            </div>
          </template>
          <template v-else>
            ...
            <span class="timer-sub">
              <span v-if="running" class="pulse">{{
                t("focus.activeSession", {
                  mode: t("focus.mode." + mode.toLowerCase()),
                })
              }}</span>
              <span v-else>{{
                t("focus.sessionType", {
      ...
      <div class="actions">
        ...
        <button
          ...
          class="action-row"
          :style="
            linkedTask ? { background: '#ffffff', borderColor: accent } : {}
          "
        ...
        >
          <div class="action-left">
            <div
              class="action-icon"
              :style="{ background: linkedTask ? '#ffffff' : 'var(--surface)' }"
            >
              <svg
                width="22"
                height="22"
                :fill="linkedTask ? accent : 'var(--muted)'"
                viewBox="0 0 256 256"
              >
                <path
            ...
            <div class="action-text">
              ...
              <p v-if="linkedTask" class="action-sub">
                {{ linkedTask.categoria }}
              </p>
            </div>
          </div>
        ...
        <button class="action-row" @click="openTimeModal">
          ...
          <span class="action-badge" :style="{ color: accent }"
            >{{ minutes }} {{ t("focus.timeLabel") }}</span
          >
        </button>
      </div>
    </div>
      ...
      >
        <div class="sheet">
          ...
          <input
            v-model="taskSearch"
            class="search-input"
            :placeholder="t('focus.searchTaskPlaceholder')"
            autofocus
          />
          <div class="task-list">
            <div v-if="filteredTasks.length === 0" class="task-empty">
              {{ t("focus.noMatchingTasks") }}
            </div>
            <button
            ...
            >
              <div
                ...
                :style="{
                  background:
                    linkedTask?.id === task.id ? accent : 'var(--border-solid)',
                }"
              ></div>
              <div class="task-info">
      ...
      >
        <div class="sheet">
          ...
          <div class="time-display" :style="{ color: accent }">
            {{ pendingMinutes }}
            <span class="time-unit">{{ t("focus.timeLabel") }}</span>
          </div>
          ...
          />
          <div class="range-labels"><span>5 min</span><span>120 min</span></div>
