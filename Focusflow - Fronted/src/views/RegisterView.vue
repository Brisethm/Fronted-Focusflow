<template>
  <div class="bg-background-light dark:bg-background-dark font-display h-screen flex overflow-hidden">
    <div class="flex flex-col items-center w-full md:w-1/2 px-4 md:px-8 lg:px-16 overflow-y-auto py-4">
      ...
      <form class="w-full max-w-sm text-left mb-4" @submit.prevent="handleSubmit">
        ...
        <div class="mb-3">
          ...
          <input id="password" name="password" type="password" maxlength="20" v-model="form.password"
            @blur="validateField('password')" :class="[
              'w-full h-12 px-4 rounded-lg bg-white dark:bg-gray-800 border focus:outline-none focus:ring-2 focus:ring-primary dark:text-white',
              errors.password
                ? 'border-red-500'
                : 'border-gray-300 dark:border-gray-600',
          ...
          <ul class="text-xs mt-2 space-y-1 text-gray-500">
            <li :class="{ 'text-green-500': form.password.length >= 8 }">
              • Mínimo 8 caracteres
            </li>
            <li :class="{ 'text-green-500': /[A-Z]/.test(form.password) }">
              • Una mayúscula
            </li>
            <li :class="{ 'text-green-500': /[a-z]/.test(form.password) }">
              • Una minúscula
            </li>
            <li :class="{ 'text-green-500': /\d/.test(form.password) }">
              • Un número
            </li>
          </ul>
        ...
        <div class="mb-6">
          <div class="flex items-center">
            ...
            <label for="terms" class="ml-2 flex items-center text-sm text-gray-900 dark:text-gray-300 cursor-pointer">
              <span>Acepto los</span>
              <a href="#" class="ml-1 font-medium text-primary hover:underline">términos y condiciones</a>
            </label>
          </div>
    ...
    <div class="hidden md:flex w-1/2 items-center justify-center bg-gray-100 dark:bg-gray-900 h-full">
      <img :src="registerIllustration" alt="Imagen decorativa" class="max-w-full h-auto" />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        ...
        // Aquí podrías redirigir al login o guardar el token si tu backend lo devuelve
      } catch (error) {
        this.loading = false;
